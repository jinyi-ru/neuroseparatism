...
class QwenProcessingParams:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "vl_target_size": ("INT", {"default": 384, "min": 8, "max": 2048, "step": 8}),
                "latent_target_size": ("INT", {"default": 1024, "min": 32, "max": 4096, "step": 32}),
                ...
                "foolproof_protection": ("BOOLEAN", {"default": False}),  # Защита от дурака
                "max_pixels": ("INT", {"default": 4194304, "min": 131072, "max": 16777216, "step": 1024}),  # 2048x2048 по умолчанию
            },
            "optional": {
    ...
    def create_params(self, vl_target_size, latent_target_size, crop_method, 
                     raw_mode, resize_method, foolproof_protection, max_pixels,
                     params_name="default"):
        params = {
            "name": params_name,
        ...
        info = f"Params '{params_name}': VL={vl_target_size}, Latent={latent_target_size}, Crop={crop_method}, Raw={raw_mode}, Resize={resize_method}, Foolproof={foolproof_protection}, MaxPixels={max_pixels}"
...
class QwenMultiProcessingParams:
    ...
    def combine_params(self, image1_params, image2_params=None, image3_params=None, image4_params=None):
        multi_params = {
...
class TextEncodeQwenImageEditAdvanced:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "global_vl_size": ("INT", {"default": 384, "min": 8, "max": 2048, "step": 8}),
                "global_latent_size": ("INT", {"default": 1024, "min": 32, "max": 4096, "step": 32}),
                "global_crop_method": (["crop", "pad", "square_pad"], {"default": "crop"}),
                "global_resize_method": (["nearest-exact", "bilinear", "area", "bicubic", "lanczos"], {"default": "lanczos"}),
                "global_raw_mode": ("BOOLEAN", {"default": False}),
                "global_foolproof_protection": ("BOOLEAN", {"default": False}),  # Защита от дурака
                "global_max_pixels": ("INT", {"default": 4194304, "min": 131072, "max": 16777216, "step": 1024}),  # 2048x2048 по умолчанию
    ...
    FUNCTION = "encode"
    CATEGORY = QWEN_CATEGORY
    DESCRIPTION = "Qwen Image Edit Advanced - Flexible processing with per-image parameters"
    
    def encode(self, clip, prompt, image1, vae=None, image2=None, image3=None, image4=None,
               multi_params=None, global_vl_size=384, global_latent_size=1024, global_crop_method="crop",
               global_resize_method="lanczos", global_raw_mode=False, global_foolproof_protection=False,
               global_max_pixels=4194304, system_prompt=""):
        ...
        for i, image in enumerate(images):
            ...
            info_lines.append(f"  Params: VL={img_vl_size}, Latent={img_latent_size}, Crop={img_crop_method}, Raw={img_raw_mode}, Foolproof={img_foolproof_protection}")
    ...
    def _process_image(self, image, target_size, resize_method, crop_method,
                      target_multiple, raw_mode, purpose="VL", 
                      foolproof_protection=False, max_pixels=4194304):
        ...
        if purpose == "VAE" and raw_mode and foolproof_protection:
            ...
            if current_pixels > max_pixels:
                ...
                image_tensor = comfy.utils.common_upscale(
                    image_tensor, new_width, new_height, 
                ...
                info_lines = []
                info_lines.append(f"Foolproof protection applied: {width}x{height} -> {new_width}x{new_height}")
                info_lines.append(f"  Scale: {scale_factor:.4f}, Pixels: {current_pixels} -> {new_width*new_height}")
        
        if raw_mode and purpose == "VAE":
            # В raw_mode для VAE только приводим к кратности
            new_width, new_height = self._make_multiple(width, height, target_multiple, crop_method)
            ...
            new_width, new_height = self._make_multiple(new_width, new_height, target_multiple, crop_method)
